...
public class ControladorTransacciones {
    ...
    ControladorCliente clienteCon = new ControladorCliente();
    ...
    public void Prestamo() {
        ...
        int pos = 0;
        for (Usuario user : RepositorUsuario.usuarios) {
            Cliente clientsa = (Cliente) user;
            if (user instanceof Cliente) {
                if (user.getPerfil().getNombre().equals(nombre) && user.getPerfil().getApellido().equals(apellido)) {
                    if (!a) {
                        ...
                        for (int k = 0; k < RepositorLibro.Libros.size(); k++) {
                            ...
                            if (libro.getIsbn().equals(isbn)) {
                                ...
                                if (!I) {
                                    ...
                                    clientsa.setPrestamoLibro(libro);
                                    libro.setDisponibilidad(false);
                                    Fecha fechas = new Fecha();
                                    fechas.setFechaPrestamos();
                                    Transaccion transaccion = new Transaccion();
                                    transaccion.setCliente(clientsa);
            ...
            if (user instanceof Cliente) {
                if (user.getPerfil().getNombre().equals(nombre) && user.getPerfil().getApellido().equals(apellido)) {
                    ...
                    if (!d) {
                        ...
                        if (!((Cliente) user).getPrestamoLibro().isEmpty()) {
                            for (Libro libro : ((Cliente) user).getPrestamoLibro()) {
                                if (libro.getTitulo().equalsIgnoreCase(titulo)) {
                                    libro.setDisponibilidad(true);
                                    ((Cliente) user).getPrestamoLibro().remove(libro);
                                    Fecha fechas = new Fecha();
                                    fechas.setFechaPrestamos();
                                    Transaccion transaccion = new Transaccion();
                                    transaccion.setCliente((Cliente) user);
                                    transaccion.setFecha(fechas);
                                    transaccion.setId(this.id());
